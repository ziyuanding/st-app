...
func parseGPUString(g string) int {
	...
	if len(parts) > 0 { val, err := strconv.Atoi(parts[len(parts)-1]); if err == nil { return val } }
	return 0
}
...
func parseJobMemToMB(m string) int {
	...
	if len(m) == 0 || m == "UNLIMITED" { return 0 }
...
func expandNodelist(nodelist string) []string {
	...
	if err != nil { return []string{nodelist} }
	return strings.Split(strings.TrimSpace(string(out)), "\n")
}

// 字符串截断辅助函数 (防止过长破坏窄屏排版)
func truncate(s string, max int) string {
	if len(s) > max { return s[:max-2] + ".." }
	return s
}